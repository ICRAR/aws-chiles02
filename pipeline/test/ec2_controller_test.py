...
def command_test03(args):
    ...
    if boto_data is not None:
        ...
        cloud_init = cloud_init.format(
                'aws-chiles02',
                boto_data[0],
                boto_data[1],
        )
        user_script = get_file_contents('node_manager_start_up.bash')
        dynamic_script = 'runuser -l ec2-user -c \'cd /home/ec2-user/aws-chiles02/pipeline/aws_chiles02 && source /home/ec2-user/virtualenv/aws-chiles02/bin/activate && ' \
                         'python startup_complete.py startup_complete us-west-2 "Hello world"\''
        user_data = get_user_data([cloud_init, user_script, dynamic_script])
        ec2_controller = EC2Controller(
                AWS_AMI_ID,
                1,
                'i2.xlarge',
