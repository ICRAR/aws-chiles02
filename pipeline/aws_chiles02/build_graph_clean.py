...
from aws_chiles02.apps import DockerCopyFromS3, DockerClean, DockerCopyCleanToS3
...
class BuildGraphClean(AbstractBuildGraph):
    ...
    def _build_s3_download(self, node_id, frequency_pair):
        ...
        s3_objects = []
        self._bucket = s3.Bucket(self._bucket_name)
        for key in self._bucket.objects.filter(Prefix='/{0}/{1}_{2}'.format(self._s3_clean_name, frequency_pair.bottom_frequency, frequency_pair.top_frequency)):
            s3_objects.append(key.key)

        s3_out_drops = [None * self._parallel_streams]
        counter = 0
        for s3_object in s3_objects:
            ...
            if s3_out_drops[counter] is not None:
                copy_from_s3.addInput(s3_out_drops[counter])
            ...
            s3_out_drops[counter] = measurement_set
