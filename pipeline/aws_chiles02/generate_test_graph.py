...
from aws_chiles02.common import get_aws_credentials, get_session_id, get_uuid
    ...
    if boto_data is not None:
        ...
        ec2_data = EC2Controller(
            ...
            tags=[
                ...
                {"Key": "Name", "Value": "DALiuGE NM - Test"},
                {"Key": "uuid", "Value": uuid},
            ],
        )
        ...
        if len(reported_running) == 0:
            ...
            data_island_manager = EC2Controller(
                ...
                tags=[
                    {"Key": "Owner", "Value": getpass.getuser()},
                    {"Key": "Name", "Value": "DALiuGE DIM - Test"},
                    {"Key": "uuid", "Value": uuid},
                ],
            )
...
def command_interactive():
    ...
    args.get(
        "spot_price_m4_large", "Spot Price for m4.large", help_text="the spot price"
    )
    args.get(
        "nodes",
        "Number of nodes",
    ...
    args.get(
        "test_directory_name",
        "The directory name for test",
        help_text="the directory name for test",
    )
    args.get(
        "use_bash",
