...
def parse_arguments():
    parser = argparse.ArgumentParser("Delete DeleteMarkers in a bucket")
...
def main():
    ...
    for count, page in enumerate(page_iterator, start=1):
        ...
        LOG.info(
            f"Page: {count}, Versions: {len(versions)}, DeleteMarkers: {len(delete_markers)}"
        )
    ...
    for item in versions:
        if item["Key"] in del_markers:
            if item["StorageClass"] != "DEEP_ARCHIVE":
                del_obj_list[item["Key"]].append(item["VersionId"])
            else:
                LOG.info(
                    f'Ignoring {item["Key"]} - {item["VersionId"]} - {item["StorageClass"]}'
                )
    ...
    for count, (del_item, value) in enumerate(del_obj_list.items(), start=1):
        ...
        LOG.info(f"Deleting {del_item} - {del_markers[del_item]}")
        if not arguments.dry_run:
