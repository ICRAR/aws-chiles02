...
class WorkToDo:
    def __init__(self, **keywords):
        ...
        self._frequency_range = get_required_frequencies(
            keywords["frequency_range"], width
        )
        self._work_to_do = []
    ...
    def calculate_work_to_do(self):
        ...
        for key in bucket.objects.filter(Prefix="{0}".format(self._s3_stats_name)):
            if key.key.endswith(".tar") and "/logs/" not in key.key:
                stats_objects.append(key.key)
        ...
        for frequency_pair in list_frequencies:
            ...
            expected_tar_file = "{0}/stats_{1}_{2}.tar".format(
                self._s3_stats_name,
                frequency_pair.bottom_frequency,
                frequency_pair.top_frequency,
            )
            uvsub_frequency = "{0}_{1}".format(
                frequency_pair.bottom_frequency, frequency_pair.top_frequency
            )
            if (
                expected_tar_file not in stats_objects
                and uvsub_frequency in uvsub_frequencies
            ):
    ...
    nodes.append(
        {
            ...
            "instance_type": "i3.4xlarge",  # on the 8xlarge av. memory eventually drops to ~40G
...
def create_and_generate(**keywords):
    ...
    if boto_data is not None:
        ...
        nodes_required, node_count = get_nodes_required(
            work_to_do.work_to_do, keywords["frequencies_per_node"], spot_price
        )
        ...
        if len(nodes_required) > 0:
            ...
            if len(reported_running) == 0:
                ...
                data_island_manager = EC2Controller(
                    ...
                    get_data_island_manager_user_data(
                        boto_data, hosts, uuid, log_level=log_level
                    ),
                    AWS_REGION,
                    tags=[
                        {"Key": "Owner", "Value": getpass.getuser()},
...
def generate_json(**keywords):
    ...
    with open(keywords.get("json_path", "/tmp/json_stats.txt"), "w") as json_file:
...
def run(command_line_):
    ...
    if config["run_type"] == "create":
        create_and_generate(
            ...
            stats_directory_name=config["stats_directory_name"],
            log_level=config["log_level"],
            uvsub_directory_name=config["uvsub_directory_name"],
            run_note=config["run_note_stats"],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "tag_name", nargs="?", default="stats", help="the tag name to execute"
    )
