...
def build_file(bucket_name):
    ...
    for top_level in top_levels:
        for key in bucket.objects.filter(
            Prefix="{0}/aa_parameter_data.yaml".format(top_level)
        ):
    ...
    for yaml_file in yaml_files:
        ...
        transfer.download_file(
            ...
            yaml_file,
            temporary_filename,
            callback=ProgressPercentage(yaml_file, s3_size),
        )
        ...
        try:
            ...
            LOG.exception("Ignoring decode error")
            return
    ...
    html_page.init(
        ...
        footer="Generated: {}".format(
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ),
    )
    html_page.p(
        (
            "The files in this directory are those generated by the CHILES pipeline using AWS. "
    ...
    html_page.li(
        "the IMAGECONCAT image cubes (into 24MHz sub-bands, in steps of 20MHz)"
    )
    html_page.ul.close()

    html_page.p(
        (
            "Each stage is in a subdirectory with a data string and in most cases "
        ...
        for key in sorted(dictionary.keys()):
            ...
            html_page.td(
                (
                    ...
                    escape(
                        clean_weighting_uv + ": {}".format(values.get("robust", ""))
    ...
    transfer.upload_file(
        ...
        extra_args={"StorageClass": "INTELLIGENT_TIERING", "ContentType": "text/html"},
    )
