...
def do_uvsub(in_dir, out_dir, out_ms, out_pngs, w_projection_planes, number_taylor_terms, model):
    ...
    Performs the UVSUB step
     Copy of uvsub_ha.py skipping the field sources, which will be retained for absorption
    ...
    LOG.info(
        'uvsub_abs(vis={0}, model={1}, out_dir={2}, out_ms={3}, w_projection_planes={4})'.format(
            in_dir,
            str(model),
            out_dir,
            out_ms,
    ...
    try:
        ...
        print str(len(model))+' models provided. Skipping '+str(ntt)+' for outlier only subtraction'
